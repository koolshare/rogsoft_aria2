...
MODULE_PAGE = ROOT / "aria2" / "webs" / "Module_aria2.asp"


class ModuleAria2ScriptOrderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_jquery_load_temporarily_hides_amd_define(self):
        guard_pattern = re.compile(
            r"__aria2_amd_define[\s\S]+?"
            r"window\.define\s*=\s*undefined[\s\S]+?"
            r'src="/js/jquery\.js"[\s\S]+?'
            r"window\.define\s*=\s*window\.__aria2_amd_define[\s\S]+?"
            r'src="/state\.js"',
            re.MULTILINE,
        )
